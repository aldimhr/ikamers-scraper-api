...
const ShopeeShop = async (url, res) => {
  // user agent
  ...
  try {
    ...
    page.on('response', async (response) => {
      if (response.url().includes('get_shop_detail?username')) {
        try {
          await response.text().then((textBody) => {
            shop = textBody;
          });
        } catch (err) {
          console.log(err);
    ...
    // WAIT RESPONSE
    await page.waitForResponse((response) => response.url().includes('get_shop_detail?username'));
    await page.waitForResponse((response) => response.url().includes('search_items?by=pop') && response.url().includes('newest=0') && !response.url().includes('only_soldout'));
    ...
    let shopproduct = (await JSON.parse(product)).items.map(({
      item_basic: {
        itemid, name, images, stock, historical_sold: sold, liked_count: likes, view_count: views, price, item_rating,
      },
    }) => ({
    ...
    // GET TOTAL PAGES
    const pages = await page.$eval('.shopee-mini-page-controller__total', (el) => el.innerText);
    ...
    if (pages > 1) {
      for (let i = 1; i <= pages; i++) {
        ...
        page.on('response', async (response) => {
          ...
          if (url.includes('search_items?by=pop') && url.includes(`newest=${30 * (i - 1)}`) && !url.includes('only_soldout')) {
            try {
              await response.text().then((textBody) => {
                product = textBody;
              });
